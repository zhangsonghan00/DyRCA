...
class IncrementalTrainer:
    ...
    def __init__(self, base_model: RCAModel, device: torch.device, 
        ...
        self.old_model = RCAModel(
            ...
            feature_dim=base_model.temporal_encoder.encoder.d_model if base_model.temporal_encoder.use_transformer else base_model.temporal_encoder.encoder.hidden_size,
    ...
    def train_incremental(self, new_node_features: torch.Tensor, new_adj_matrix: torch.Tensor, 
                         new_anomaly_labels: torch.Tensor, new_root_cause_labels: torch.Tensor,
                         old_samples_ratio: float = 0.2) -> float:
        """执行增量训练步骤，结合新知识和旧知识"""
        self.base_model.train()
        
        # 1. 使用旧模型生成旧样本的预测（知识蒸馏）
        with torch.no_grad():
            old_outputs = self.old_model(new_node_features.to(self.device), new_adj_matrix.to(self.device))
            old_root_cause_probs = old_outputs['root_cause_probs']  # [batch_size, num_services]
            old_anomaly_probs = torch.softmax(old_outputs['anomaly_score'], dim=1)  # [batch_size, 2]
        
        # 2. 计算新数据上的损失
        new_outputs = self.base_model(new_node_features.to(self.device), new_adj_matrix.to(self.device))
        new_root_cause_probs = new_outputs['root_cause_probs']  # [batch_size, num_services]
        new_anomaly_probs = torch.softmax(new_outputs['anomaly_score'], dim=1)  # [batch_size, 2]
        ...
        anomaly_loss = self.trainer.anomaly_criterion(new_outputs['anomaly_score'], new_anomaly_labels)
...
# 示例：增量训练
    def incremental_train_demo():
        # 创建增量训练器
        incremental_trainer = IncrementalTrainer(model, device)
        
        # 假设我们有新数据
        new_batch_size = 16
        new_node_features = torch.randn(new_batch_size, num_services, window_size, feature_dim)
        ...
        for epoch in range(incremental_epochs):
            ...
            if (epoch + 1) % 5 == 0:
                print(f"Incremental Epoch {epoch+1}/{incremental_epochs}, Loss: {loss:.4f}")
